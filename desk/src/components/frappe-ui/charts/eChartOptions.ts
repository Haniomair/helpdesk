...
export const SUBTITLE_HEIGHT = 20
...
export default function useEchartsOptions(config: AxisChartConfig) {
  ...
  return {
    ...
    tooltip: {
      ...
      formatter: (params: Object | Array<Object>) => {
        ...
        if (!Array.isArray(params)) {
          ...
          return `
                <div style="${isRtl ? 'direction: rtl;' : ''}" class="flex items-center items-reverse justify-between gap-5">
                  <div>${p.name}</div>
        ...
        if (Array.isArray(params)) {
          const t = params.map((p, idx) => {
            ...
            return `
              <div class="flex flex-col gap-1" style="${isRtl ? 'direction: rtl;' : ''}">
                ${idx == 0 ? `<div class="mb-2">${formattedX}</div>` : ''}
                <div class="flex items-center justify-between gap-5">
                  <div class="flex gap-1 items-center">
                    ${p.marker}
                    <div>${__(formatLabel(p.seriesName))}:</div>
                  </div>
    ...
    legend: {
      ...
      itemGap: isRtl ? 40 : 20,
      padding: [0, 25],
      align: isRtl ? 'right' : 'auto',
      formatter: function (name: string) {
        return `{a|${__(formatLabel(name))}}`
      },
      textStyle: {
        color: '#000',
        overflow: 'truncate',
...
function getSubtitle(subtitle: string) {
  
  if (!subtitle) return ''

  let _subtitle = subtitle;
  if (Array.isArray(subtitle)) {
    _subtitle = subtitle[0];
  }

  // extract value from subtitle if it contains a value
  const valueMatch = _subtitle.match(/\[(.*?)\]/g)
  let value = ''
  if (valueMatch) {
    value = valueMatch[0].replace(/[\[\]]/g, '').trim()
    _subtitle = _subtitle.replace(valueMatch[0], '').trim()
  }

  return `${__(_subtitle)} ${value ? ` ${value}` : ''}`

}

function calculateYAxisNamePadding(name: string, options: any = {}) {
  ...
  const dynamicPadding = Math.min(
    Math.floor(basePadding + textLength * scaleFactor),
    maxPadding
  );
...
function getYAxisOptions(config: AxisChartConfig) {
  ...
  let secondaryYAxisOptions = {
    ...
    scale: false,
    boundaryGap: ['0%', '1%'],
    name: `${__(config.y2Axis?.title)} ↑`,
    nameLocation: 'end',
    nameTextStyle: {
      align: 'right',
      verticalAlign: 'top',
      padding: [0, 0, 0, calculateYAxisNamePadding(config.y2Axis?.title, { scaleFactor: getDirection() == 'rtl' ? 4 : 2 })],
      // color: '#000',
      backgroundColor: '#fff',
    },
    nameGap: 10,
